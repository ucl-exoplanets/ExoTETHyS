...
from limb_darkening_laws import *
import copy
import pickle
...
def get_grid_parameters(stellar_models_grid):
	if stellar_models_grid=='Phoenix_2018':
		path = '/Users/pepe/Desktop/PHOENIX_Claret/COND2018pickle/'
		files = [f for f in os.listdir(path) if f.startswith('teff')]
		star_params_grid = np.zeros((len(files),3))
		for i in range(len(files)):
			star_params_grid[i,:] = np.array(( np.float(files[i][4:9]), np.float(files[i][14:18]), np.float(files[i][21:24]) ))
	elif stellar_models_grid=='Phoenix_2012_13':
		path = '/Users/pepe/Desktop/PHOENIX_Claret/COND2012_13_pickle/'
		files = [f for f in os.listdir(path) if f.startswith('teff')]
		star_params_grid = np.zeros((len(files),3))
		for i in range(len(files)):
			star_params_grid[i,:] = np.array(( np.float(files[i][4:9]), np.float(files[i][14:18]), np.float(files[i][21:24]) ))
...
def get_passband(passband, stellar_models_grid):
	path = '/Users/pepe/Desktop/PHOENIX_Claret/Passbands/'
	ext = '.pass'
	[pb, check] = read_as_numpy_array(path+passband+ext)
	if not check:
...
#	c3 = params[2]
#	c4 = params[3]
#	model = 1.0 - c1*(1.0-mucut**0.5) - c2*(1.0-mucut**1.0) - c3*(1.0-mucut**1.5) - c4*(1.0-mucut**2.0)
#	return np.sum(((intscut-model)*weights)**2) / np.sum(weights**2.0)
...
def process_configuration(input_dict):
	...
	if calculation_type=='individual':
		...
		final_passbands = neighbour_limb_darkening_coefficients[i0]['passbands'].keys()
		...
		with open(output_path+'target_ldc.pickle' , 'wb') as out1:
...
input_dict = read_configuration('input_ldc.txt')
if check_configuration(input_dict):
	[limb_darkening_coefficients, integrated_intensities] = process_configuration(input_dict)
